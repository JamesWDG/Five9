...
const page = () => {
    ...
    const fetchHomeBannerSecData = async () => {
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/dashboard/cms/our-services-page/Banner_Section/${id}`)
        ...
        <>
            ...
            <section className="dashboard-inner-pages">
                <div className="container-fluid">
                    <h2 className="db-hd">Edit {pageName} {parentName} {heading} , {subheading} </h2>
                    <form action="" className="db-form" encType="multipart/form-data" onSubmit={(e) => {
                        e.preventDefault()
                        updateHomeBannerSecData()
                    }}>
                        <label htmlFor="main_heading" className="db-hd-mini">{heading}</label>
                        <input type="text" name="heading" id="main_heading" defaultValue={headingValue} onChange={(e) => setheadingValue(e.target.value)} className="header-input" placeholder="Mini Heading Goes here" required />
