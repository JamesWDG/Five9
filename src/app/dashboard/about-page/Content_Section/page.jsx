...
const page = () => {
    ...
    const fetchContentSecDataSecData = async () => {
        try {
            console.log("id ya hay ====>>>", id);
            const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/dashboard/cms/about-us-page/Content_Section/${id}`)
            ...
            Swal.fire({
                ...
                icon: "error",
                title: "Oops!",
                text: "Cannot fetch content sec Data.",
            });
        }
        finally {
    ...
    const updateContentSecDataSecData = async () => {
        try {
            const formData = new FormData()
            formData.append("heading", value1)
            formData.append("title", value2)

            const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/dashboard/cms/about-us-page/Banner_Section/${id}`,
                {
                    method: "POST",
                    body: formData,
                })
            ...
            <section className="dashboard-inner-pages">
                <div className="container-fluid">
                    ...
                    <form action="" className="db-form" encType="multipart/form-data" onSubmit={(e) => {
                        ...
                        <label htmlFor="title" className="db-hd-mini mt-5">{heading2}</label>
                        <textarea type="text" name="title" id="title" defaultValue={value2} onChange={(e) => setvalue2(e.target.value)} className="header-input" placeholder="Main Heading Goes here" required />
