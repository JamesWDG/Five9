...
const page = () => {
    ...
    const fetchContactTaglineSecData = async () => {
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/dashboard/cms/contact-us-page/Let_Talk_Section/${id}`)
            ...
            <section className="dashboard-inner-pages">
                <div className="container-fluid">
                    ...
                    <form action="" className="db-form" encType="multipart/form-data" onSubmit={(e) => {
                        ...
                        <input type="text" name="sub_heading" id="sub_heading" defaultValue={subheadingValue} onChange={(e) => setsubheadingValue(e.target.value)} className="header-input" placeholder="Mini Heading Goes here" required />
                        <label htmlFor="para" className="db-hd-mini mt-5">{para}</label>
                        <textarea type="text" name="para" id="para" defaultValue={paraValue} onChange={(e) => setparaValue(e.target.value)} className="header-input" placeholder="Mini Heading Goes here" required />
